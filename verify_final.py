...
def run():
    screenshots_dir = 'verification_screenshots_final'
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            # Mobile Bank Statements (Previous Uploads Card View)
            # Re-login or bypass with auth cookie if session was shared, but easier to just go to parent or public
            # Since I used localStorage for parent, I can use that
            mobile_page.goto('http://localhost:5173/parent/login')
            mobile_page.evaluate('() => { localStorage.setItem("parentAuth", "true"); }')
            mobile_page.goto('http://localhost:5173/parent/dashboard')
            mobile_page.wait_for_selector('h1', timeout=10000)
            mobile_page.screenshot(path=os.path.join(screenshots_dir, '08_mobile_parent.png'))
            print('08_mobile_parent.png captured')
